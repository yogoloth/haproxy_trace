...
./get_sess ${haproxy_socket} | grep -v fd=-1 | awk '{print $3" "$4" "$5" "$16" "$17 }'| sed 's/s0=\[.*fd=\([0-9]\+\).*\].*s1=.*\[.*fd=\([0-9]\+\).*\]/\1 \2/g' >$tmp_sess


./haproxy_trace $tmp_sess $tmp_socket_file $tmp_proc_file 
